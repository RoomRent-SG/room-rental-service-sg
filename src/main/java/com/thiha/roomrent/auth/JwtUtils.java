...
public class JwtUtils {
    ...
    public String generateJwtToken(UserModel user, Boolean isRefreshToken) {
        ...
        return Jwts.builder().subject(user.getId().toString()).issuedAt(DateTimeHandler.getUTCNow()).expiration(expiryDate).signWith(getSigningKey(), Jwts.SIG.HS256).claim("user", createJwtClaims(user)).compact();
    }
